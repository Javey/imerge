...
Layout.prototype = {
    ...
    init: function() {
        ...
        this.createRoot();
        var root = this.root;
        _.each(this.group.center, function(value) {
            console.warn('Center is not supported: ' + value.file);
        });
        delete this.group.center;
        _.each(this.group, function(value, key) {
            if (key === 'none') {
                ...
                throw new Error('"' + key + '" is not supported. One file is "' + value[0].file + '"');
            }
        });
    },
