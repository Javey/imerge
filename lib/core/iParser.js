...
var IParser = module.exports = function(files, defaults, options) {
    this.files = files;
...
IParser.prototype = {
    ...
    _parse: function(content, css_file) {
        ...
        obj.stylesheet.rules.forEach(function(rule) {
            if (rule.type === 'rule') {
                ...
                if (opt.all && !_.isEmpty(setImage)) {
                    merge = utils.defaults.merge;
                }
                if (merge) {
                    _.each(setImage, function(value, key) {
                        if (key === 'ie6') {
                            self._setConfigByRule(merge + utils.defaults.ie6Suffix, bgDecls, true, css_file);
                        } else {
                            self._setConfigByRule(merge, bgDecls, false, css_file);
                        }
                    });
                }
            }
    ...
    _setConfigByRule: function(merge, bgDecls, isHack, css_file) {
        var config = {},
            self = this;
    ...
    _setConfig: function(merge, url, config) {
        ...
        if (!this.config[merge][url]) {
            ...
            var oldConfig = this.config[merge][url];
