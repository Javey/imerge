...
var IMerge = module.exports = function(options, pathFilter) {
    ...
    this.pathFilter = _.defaults(pathFilter || {}, {
        ...
        imageAbsolutePath: function(file, config) {
            if (relativeReg.test(config.bg_origin_path)) {
                return path.resolve(path.dirname(config.css_path), config.bg_origin_path);
            }

            return path.join(opt.from, opt.webroot, config.bg_origin_path);
        },
        imageRelativePath: function(file, config) {
            return config.bg_origin_path;
        },
    ...
    kvData: function(data) {
        ...
        _.each(data, function(sprite) {
            ...
            _.each(sprite.images, function(image) {
                var imageFile = imagePathFilter(image.file, image.config);
